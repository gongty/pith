...
async function fetchRss(source) {
  ...
  while ((match = itemRegex.exec(xml)) !== null) {
    ...
    const richContent = contentEncoded || desc;
    const fullContent = stripHtml(richContent);
    items.push({
      title: stripHtml(title),
      url: link.trim(),
      summary: stripHtml(desc).slice(0, 1000),
      fullContent: fullContent.length > 200 ? fullContent : '',
  ...
  if (!items.length) {
    ...
    while ((match = entryRegex.exec(xml)) !== null) {
      ...
      const atomContent = (block.match(/<content[^>]*>(?:<!\[CDATA\[)?([\s\S]*?)(?:\]\]>)?<\/content>/i) || [])[1] || '';
      const updated = (block.match(/<updated[^>]*>([\s\S]*?)<\/updated>/i) || [])[1]
                   || (block.match(/<published[^>]*>([\s\S]*?)<\/published>/i) || [])[1] || '';
      const richAtom = atomContent || summary;
      const fullAtom = stripHtml(richAtom);
      items.push({
        title: stripHtml(title),
        url: link.trim(),
        summary: stripHtml(summary).slice(0, 1000),
        fullContent: fullAtom.length > 200 ? fullAtom : '',
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', code => {
      ...
      const items = arr.map(it => {
        const content = it.content || it.summary || it.description || '';
        return {
        title: it.title || it.name || '',
        url: it.url || it.link || '',
        summary: content.slice(0, 1000),
        fullContent: content.length > 200 ? content : '',
