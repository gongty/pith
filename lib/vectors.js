...
let _dataRoot = process.env.WIKI_DATA_DIR || path.join(ROOT, 'data');
let WIKI_DIR = path.join(_dataRoot, 'wiki');
let VEC_DIR = path.join(_dataRoot, 'vectors');
let INDEX_PATH = path.join(VEC_DIR, 'index.jsonl');
let META_PATH = path.join(VEC_DIR, 'meta.json');

function __setDataRoot(root) {
  _dataRoot = root;
  WIKI_DIR = path.join(_dataRoot, 'wiki');
  VEC_DIR = path.join(_dataRoot, 'vectors');
  INDEX_PATH = path.join(VEC_DIR, 'index.jsonl');
  META_PATH = path.join(VEC_DIR, 'meta.json');
}
...
// ── 最近一次 buildVectorIndex 的运行态（供 /api/wiki/vectors/stats 暴露）──
// fire-and-forget 触发的重建若失败，前端除了 console 无法感知，这里给个轮询出口。
let __lastBuildStatus = {
  running: false,
  startedAt: null,
  finishedAt: null,
  error: null,
  result: null // 成功时填 { total, added, updated, removed, skipped, durationMs, errors }
};
function getBuildStatus() {
  // 返回浅拷贝，避免外部误改内部状态
  return {
    running: __lastBuildStatus.running,
    startedAt: __lastBuildStatus.startedAt,
    finishedAt: __lastBuildStatus.finishedAt,
    error: __lastBuildStatus.error,
    result: __lastBuildStatus.result
  };
}
...
async function buildVectorIndex({ force = false, paths = null } = {}) {
  ...
  // 标记 running 前先把上次错误清了；失败时会回填 error。
  __lastBuildStatus = {
    running: true,
  ...
  try {
    const r = await _buildVectorIndexInner({ force, paths, started });
    __lastBuildStatus = {
      running: false,
      startedAt: __lastBuildStatus.startedAt,
...
async function _buildVectorIndexInner({ force = false, paths = null, started } = {}) {
  const cfg = getServerConfig();
