...
export function openSearch() {
  ...
  state.searchIdx = -1;
  $('searchResults').innerHTML = '<div class="search-empty-msg">' + h(t('search.empty')) + '</div>';
}
...
function aiRow(q) {
  return '<div class="search-ask-ai" onclick="askFromSearch()">' +
    ...
    '<span class="search-ask-ai-text">' + h(t('search.askAI', { q })) + '</span>' +
...
export function initSearchInput() {
  $('searchInput').addEventListener('input', e => {
    ...
    if (!q) { $('searchResults').innerHTML = '<div class="search-empty-msg">' + h(t('search.empty')) + '</div>'; return; }
    state.st = setTimeout(async () => {
      try {
        const r = await api('/api/search?q=' + encodeURIComponent(q)); const c = $('searchResults');
        if (!r || !r.length) {
          c.innerHTML = '<div class="search-empty-msg">' + h(t('search.notFound', { q })) + '</div>' + aiRow(q);
          return;
        }
        const shown = Math.min(r.length, 8);
        const countHint = '<div class="search-count">' + h(t('search.count', { shown, total: r.length })) + '</div>';
      ...
      } catch { $('searchResults').innerHTML = '<div class="search-empty-msg">' + h(t('search.error')) + '</div>'; }
    }, 200);
  });
}
