...
export async function rGraph(c) {
  ...
  try {
    ...
    s += '</div></div>';
    c.innerHTML = s;
    if (data.nodes.length >= 2) requestAnimationFrame(() => { const cv = document.getElementById('fgCanvas'); if (cv) initFG(cv, data, true); });
...
export function initFG(canvas, data, full) {
  ...
  const nodes = data.nodes.map((n, i) => {
    ...
    const radius = Math.max(4, Math.min(14, 4 + (cc[n.id] || 0) * 2));
  ...
  function draw() {
    ...
    ctx.font = '12px ' + ff; ctx.textAlign = 'center';
    for (const n of nodes) {
      const text = n.keyword || (n.name ? n.name.slice(0, 6) : '');
      if (!text) continue;
    ...
    // Hover tooltip — show full article title
    if (hov) {
      const text = hov.name || hov.id;
      ctx.font = '12px ' + ff;
  ...
  canvas.ontouchstart = e => {
    ...
    const w = s2w(mx, my);
    for (const n of nodes) { const dx = w.x - n.x, dy = w.y - n.y; if (dx * dx + dy * dy < (n.radius + 8) ** 2) { drag = n; dsx = mx; dsy = my; return; } }
    pan = true; psx = mx; psy = my; pox = px; poy = py;
  };
  canvas.ontouchmove = e => {
    ...
    const mx = t.clientX - r.left, my = t.clientY - r.top;
    if (drag) { const w = s2w(mx, my); drag.x = w.x; drag.y = w.y; drag.vx = 0; drag.vy = 0; draw(); return; }
    if (pan) { px = pox + (mx - psx); py = poy + (my - psy); state.gs.px = px; state.gs.py = py; draw(); }
  };
  canvas.ontouchend = e => {
    ...
    if (drag) {
      ...
      if (Math.sqrt((mx - dsx) ** 2 + (my - dsy) ** 2) < 8) go('#/article/' + drag.id);
      drag = null;
    }
    if (pan) pan = false;
  };
